...
class VCLLoggingViewController: UIViewController
{
    ...
    var vlcLoggingName: String {
        return String(describing: type(of: self))
    }
    
    private func logVCL(_ msg: String) {
        if instanceCount == nil {
            instanceCount =  VCLLoggingViewController.bumpInstanceCount(for: vlcLoggingName)
        }
        print("\(VCLLoggingViewController.logPrefix(for: vlcLoggingName))(\(instanceCount!)) \(msg)")
    }
